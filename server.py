...
CANNED_CATEGORIES = ['ירי רקטות וטילים', 'רעידת אדמה', 'חומרים מסוכנים', 'אירוע ביטחוני',
                     'מזג אוויר קיצון', 'שריפה', 'תשתיות', 'הרגעה ועדכון כללי', 'אחר']
...
ROLE_NAMES = {'admin': 'מנהל מערכת', 'lead': 'מש"ק הסברה', 'user': 'משתמש', 'viewer': 'צפייה בלבד'}
...
# תבניות בנק ההודעות (title, category, audience, body). מוזרקות ב-init_db לפי
# כותרת — תבנית שכבר קיימת (או שנמחקה) לא מוזרקת שוב, כך שעריכות נשמרות.
# ה-[סוגריים] הם שדות למילוי במחולל ההודעות.
CANNED_SEEDS = [
    # --- ירי רקטות וטילים ---
    ...
    ('עדכון מצב — ירי רקטות', 'ירי רקטות וטילים', 'ציבור',
     'עדכון לתושבי [אזור] נכון לשעה [שעה]: [תיאור המצב — נפילות / יירוטים / אין נפגעים]. '
     'הנחיות פיקוד העורף בתוקף — הישארו בקרבת מרחב מוגן. נעדכן בכל שינוי בערוצים הרשמיים.'),
    ('נפילה בשטח בנוי — ירי רקטות', 'ירי רקטות וטילים', 'ציבור',
     'דווח על נפילה ב-[מיקום]. כוחות ההצלה בדרכם לזירה — נא להתרחק מהמקום ולאפשר את '
     'עבודתם. זיהיתם נפל, רסיס או חפץ חשוד — אל תיגעו בו ודווחו מיד למשטרה במוקד 100.'),
    ('הסרת הנחיות — סיום אירוע ירי', 'ירי רקטות וטילים', 'ציבור',
     'פיקוד העורף מסיר את ההגבלות באזור [אזור] החל מהשעה [שעה]. ניתן לחזור לשגרה מלאה. '
     '[הנחיה בנושא לימודים / התקהלויות אם רלוונטי]. תודה על שיתוף הפעולה.'),
    # --- רעידת אדמה ---
    ...
    ('עדכון מצב — רעידות משנה', 'רעידת אדמה', 'ציבור',
     'לתושבי [אזור]: בשעות הקרובות ייתכנו רעידות משנה. אל תיכנסו למבנים שנפגעו או נסדקו. '
     'שהו בשטח פתוח ככל הניתן. נזק למבנה או לתשתית — דווחו למוקד העירוני [טלפון המוקד].'),
    ('סיום אירוע — רעידת אדמה', 'רעידת אדמה', 'ציבור',
     'עדכון לתושבי [אזור]: ניתן לחזור למבנים שלא נצפה בהם נזק. מבנה סדוק או פגוע — אין '
     'להיכנס אליו, ודווחו למוקד העירוני [טלפון המוקד]. עקבו אחר הנחיות בערוצים הרשמיים.'),
    # --- חומרים מסוכנים ---
    ...
    ('עדכון מצב — חומרים מסוכנים', 'חומרים מסוכנים', 'ציבור',
     'עדכון: האירוע ב-[מיקום] בטיפול כוחות הכיבוי וההצלה. ההנחיה לתושבי [אזור] להישאר '
     'במבנים סגורים עדיין בתוקף, צפי עדכון נוסף בשעה [שעה משוערת]. אין להתקרב לאזור.'),
    ('הסרת הנחיות — חומרים מסוכנים', 'חומרים מסוכנים', 'ציבור',
     'האירוע ב-[מיקום] הסתיים. תושבי [אזור] — ניתן לפתוח חלונות, להפעיל מזגנים ולצאת '
     'מהמבנים. תחושת מחנק או גירוי בדרכי הנשימה — פנו למד"א במוקד 101.'),
    # --- אירוע ביטחוני ---
    ('אירוע ביטחוני — הודעה ראשונית', 'אירוע ביטחוני', 'ציבור',
     'מתרחש אירוע ביטחוני ב-[מיקום]. תושבי [אזור] מתבקשים להישאר בבתים ולנעול דלתות '
     'וחלונות עד להודעה חדשה. אל תפיצו שמועות או מיקומי כוחות. עדכונים בערוצים הרשמיים בלבד.'),
    ('אירוע ביטחוני — סיום', 'אירוע ביטחוני', 'ציבור',
     'האירוע הביטחוני ב-[מיקום] הסתיים. אין מניעה לצאת מהבתים ולחזור לשגרה. '
     '[הנחיות משלימות אם נדרש]. תודה על שיתוף הפעולה ושמירת ההנחיות.'),
    # --- מזג אוויר קיצון ---
    ('עומס חום — הנחיות לציבור', 'מזג אוויר קיצון', 'ציבור',
     'בתאריכים [תאריכים] צפוי עומס חום כבד. הימנעו משהייה בשמש בין השעות [שעות], הקפידו '
     'על שתיית מים, והשגיחו על קשישים, ילדים וחולים. במצוקה רפואית חייגו 101.'),
    ('שיטפונות — אזהרה', 'מזג אוויר קיצון', 'ציבור',
     'בעקבות הגשמים צפויות הצפות ושיטפונות ב-[אזורים]. התרחקו מנחלים, ואדיות ואפיקי ניקוז. '
     'אין לחצות כביש מוצף — גם לא ברכב. מקרה חירום: 102 / המוקד העירוני [טלפון המוקד].'),
    ('סופה / שלג — היערכות', 'מזג אוויר קיצון', 'ציבור',
     'החל מ-[שעה/תאריך] צפויה [סופה / שלג / רוחות עזות]. היערכו מראש: הצטיידו במים, מזון '
     'ותאורת חירום, קשרו חפצים בחצרות והימנעו מנסיעות לא הכרחיות. מוקד חירום: [טלפון המוקד].'),
    # --- שריפה ---
    ('שריפה — הודעה ראשונית ופינוי', 'שריפה', 'ציבור',
     'שריפה פעילה באזור [מיקום]. תושבי [רחובות/שכונות] מתבקשים להתפנות דרך [ציר פינוי] '
     'אל [נקודת קליטה]. אין להתקרב לאזור השריפה. סייעו לשכנים המתקשים להתפנות.'),
    ('שריפה — סיום אירוע', 'שריפה', 'ציבור',
     'השריפה באזור [מיקום] נכבתה והאירוע הסתיים. המפונים רשאים לשוב לבתיהם. '
     'נזק למבנה או לרכוש — דווחו למוקד העירוני [טלפון המוקד]. בריאות לקויה בשל עשן — חייגו 101.'),
    # --- תשתיות ---
    ('הפסקת חשמל ממושכת', 'תשתיות', 'ציבור',
     'הפסקת חשמל באזור [אזור] בעקבות [סיבה]. צפי חזרת האספקה: [שעה משוערת]. '
     'תושבים המחוברים למכשירים רפואיים תלויי חשמל — פנו למוקד העירוני [טלפון המוקד]. '
     'דיווח לחברת החשמל: 103.'),
    ('שיבוש אספקת מים', 'תשתיות', 'ציבור',
     'שיבוש באספקת המים באזור [אזור] בעקבות [סיבה]. חלוקת מים לתושבים תתקיים ב-[נקודות חלוקה]. '
     'צפי חזרת האספקה: [שעה משוערת]. תאגיד המים [שם התאגיד]: [טלפון].'),
    # --- הרגעה ועדכון כללי + קהלים פנימיים ---
    ...
    ('תמונת מצב לרשויות', 'הרגעה ועדכון כללי', 'רשויות',
     'תמונת מצב לשעה [שעה]: [תיאור האירוע]. פעולות שבוצעו: [פירוט]. הנחיות בתוקף: [הנחיות]. '
     'צפי עדכון הבא: [שעה]. נא לוודא הפצה לגורמים הרלוונטיים ברשות.'),
    ('עדכון למוקד — שאלות ותשובות', 'הרגעה ועדכון כללי', 'מוקד',
     'לצוות המוקד — מענה מאושר לפניות בנושא [נושא]: [נוסח המענה לתושב]. '
     'פניות חריגות או שאלות ללא מענה — יש להפנות ל-[גורם מטפל].'),
    ('קו מסרים לדוברים', 'הרגעה ועדכון כללי', 'דוברים',
     'קו מסרים מאושר לאירוע [תיאור]: 1. [מסר מרכזי]. 2. [מסר משני]. '
     'אין להתייחס ל-[נושא רגיש] עד לעדכון נוסף. שאלות — [איש קשר].'),
    ('עדכון ליקל"רים', 'הרגעה ועדכון כללי', 'יקל"רים',
     'עדכון לשעה [שעה]: [סטטוס האירוע והנחיות בתוקף]. נא לוודא הפצה לתושבים בערוצי הרשות '
     'ולדווח על פערי מידע או שמועות ל-[איש קשר].'),
]
...
def init_db():
    ...
    # תבניות הבנק — ניתנות לעריכה/מחיקה מהמסך. ההשוואה מול כל השורות כולל
    # active=0, כדי שתבנית שנמחקה לא תוזרק מחדש באתחול הבא.
    existing_titles = {r['title'] for r in c.execute('SELECT title FROM canned_messages')}
    for title, cat, aud, body in CANNED_SEEDS:
